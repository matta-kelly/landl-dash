...
def se_home():
    """
    Generates the layout for the Surf Expo homepage.

    Returns:
        dash.html.Div: Layout for the page.
    """
    # Access Surf Expo data processing
    from data_preprocessing.se_processing import process_surf_expo_data

    # Get the filtered Surf Expo data (as a DataFrame) and stats
    surf_expo_data_result = process_surf_expo_data()
    stats = surf_expo_data_result['stats']
    surf_expo_data = surf_expo_data_result['merged_data']

    # Compute Clothing vs. Jewelry data
    clothing_data = surf_expo_data[surf_expo_data["Category Group"] == "CLOTHING"]
    jewelry_data = surf_expo_data[surf_expo_data["Category Group"] == "JEWELRY"]
    ...
    spsu25_distribution = surf_expo_data["SPSU25 Status"].value_counts(normalize=True).reset_index()
    ...
    return html.Div(
        [
            html.H1("Welcome to the Surf Expo Home Page", style={"textAlign": "center"}),
